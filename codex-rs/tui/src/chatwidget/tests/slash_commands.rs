...
/// normal Codex `AppServerClient` path. Also proves the ace pipeline runs: the turn becomes
/// a raw outcome record (no model call), which feeds the session's `saved` total once it
/// ages out of the most recent turn, without printing a per-turn line.
#[cfg(unix)]
#[tokio::test]
...
"#,
    ...
    let completed = loop {
            ...
            .expect("channel closed before turn completed")
        {
            AppEvent::ClaudeCodeTurnCompleted {
                ...
                error,
                outcome_record,
            } => break (text, session_id, error, outcome_record),
            _ => continue,
        }
    };
    ...
    let (text, session_id, error, outcome_record) = completed;
    ...
    // No model call involved: the record is built deterministically from the raw turn.
    assert_eq!(
        outcome_record.as_deref(),
        Some("User: hello claude\nAssistant: fake reply")
    );
    ...
    chat.handle_claude_code_turn_completed(text, session_id, error, outcome_record);
    assert!(!chat.is_claude_code_turn_running());
