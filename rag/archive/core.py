        ...
        if not os.path.exists(self.persist_dir) or self.config.get("force_reindex", False):
            ...
            for path in self.document_paths:
                ...
                text_splitter = RecursiveCharacterTextSplitter(
                    ...
                    # Its job is to make the chunks "smarter". The RecursiveCharacterTextSplitter tries to split the text at the first separator in the list (\n\n - a new paragraph). If a chunk is still too big, it tries the next separator (\n - a new line), and so on. It's a prioritized list of where to cut the text to avoid breaking sentences in the middle.
                    separators=["\n\n", "\n", ". ", "،", " ", ""]
                )
